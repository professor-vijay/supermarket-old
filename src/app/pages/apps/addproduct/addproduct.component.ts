...
export class AddproductComponent implements OnInit {
  ...
  loginfo: any;;
  product: any = { id: 0, name: '', barCode: null, description: "", brand: "", categoryId: 0, taxGroupId: 0, productTypeId: 0, unitId: 0,  price: null, productCode: null, CompanyId: 0, action: "", }
...
// formatter = (x: { name: string }) => x.name;
  ...
  ngOnInit(): void {
    ...
    this.getCategories();
    // this.getvendors();
    this.product = { id: 0, name: '', barCode: null, description: "", brand: "", categoryId: 0, taxGroupId: 0, productTypeId: 0, unitId: 0,  price: null, productCode: null, CompanyId: this.loginfo.companyId, action: "",  }
    })
    ...
    if (this.prodid != '0') {
      this.Auth.getproductbyid(this.prodid).subscribe(data => {
        ...
        this.barcodes.forEach(bc => {
          bc.vids = []
          bc.com_code = ''
          this.barcodevariants.filter(x => x.barcodeId == bc.id).forEach(bcv => {
  ...
  addProduct() {
    ...
    // // console.log(this.submitted, this.product);
    if (this.validation()) {
      if (this.product.id == 0) {
        ...
        // this.Auth.updateProduct(this.product).subscribe(data => {
          ...
          // if (this.datasavetype == "1") {
            this.Auth.addproduct_l({ product: this.product, variantcombinations: this.combinations },20,this.loginfo.storeId).subscribe(data => {
              // // console.log(data)
              this.Auth.addbarcodeproductdb(data["stocks"]).subscribe(data2 => {
        ...
        // this.Auth.updateProduct({ product: this.product, variantcombinations: this.combinations }).subscribe(data => {
          // console.log(data)
          // if (this.datasavetype == "1") {
            this.Auth.updateproduct_l({ product: this.product, variantcombinations: this.combinations },20).subscribe(data => {
              // console.log(data)
  ...
  getcategoryvariants() {
    ...
    this.Auth.getcategoryvariants(this.product.categoryId).subscribe(data => {
      ...
      if (this.barcodes && this.barcodevariants)
        this.setcvselect();
    })
  }
  setcvselect() {
