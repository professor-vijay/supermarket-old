...
export class OrderModule {
    ...
    rdaddproduct(product) {
        this.Items.push(new OrderItemModule(product))
        // this.setbillamount()
        this.setupdatedreceivedbillamount();
    }
    setbillamount() {
        console.log("SALE : SETBILLAMOUNT")
        this.BillAmount = 0;
        this.DiscAmount = 0;
        this.Subtotal = 0
        this.TaxAmount = 0
    ...
    setreceivedbillamount() {
        console.log("RECIEVE : SETBILLAMOUNT")
        this.BillAmount = 0;
        this.DiscAmount = 0;
        this.Subtotal = 0
        this.TaxAmount = 0
        ...
        this.Tax3 = 0
        this.Items.forEach(item => {
            if(item.OrderQuantity < item.ReceivedQuantity) {
                item.OrderQuantity = item.ReceivedQuantity
            }
            item.TotalAmount = item.Price * item.ReceivedQuantity
            if (item.IsInclusive) {
                item.TotalAmount = item.TotalAmount - item.TotalAmount * (item.Tax1 + item.Tax2 + item.Tax3) / 100
            }
            item.TaxAmount1 = item.Tax1 * item.TotalAmount / 100
...
export class OrderItemModule {
    ...
    constructor(product) {
        ...
        this.ReceivedQuantity = product.rQuantity;
